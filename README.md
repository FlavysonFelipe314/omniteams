...
- Seletor de período com digitação em `dd/mm/aaaa`, calendário contínuo de início e fim e atalhos para períodos frequentes.
- O seletor de período abre sobre toda a tela, sem ser recortado pela barra lateral.
...
- Story points concluídos do ranking usam a data de resolução dentro do período selecionado, seguindo o mesmo critério do comparativo de evolução.
...
- Inclusão e remoção rápida de períodos; seletores e gráficos são organizados em blocos responsivos de até três intervalos por linha.
...
- Períodos longos são consultados em blocos semanais; timeouts e respostas grandes são repartidos automaticamente em blocos menores.
- Criação, edição e exclusão de apontamentos.

### Gestão

- Grade **Horas por Pessoa e Dia**, com total por colaborador e por dia, rolagem horizontal e nomes fixos. Clique em uma célula com horas para visualizar os cards apontados naquele dia.
...
- Filtro **Epic/Pai** na Gestão e no Meu Perfil, além de filtro e coluna opcional na exportação. Usa o pai direto do Jira (para uma subtarefa, a tarefa pai), com compatibilidade para Epic Link legado. Cards sem vínculo aparecem como **Sem Epic/Pai**.
...
- Consulta independente por período de homologação e projetos, dividida em blocos menores para evitar timeout.
...
- Cópia de uma linha, de uma categoria ou da Release Note completa no formato `1º - MELHORIA: Resumo do card (CHAVE-123);`.
- A aba utiliza apenas seus próprios filtros e ocupa toda a largura, sem o seletor lateral de colaboradores.
...
- alinhamento dos story points concluídos pela data de resolução do período;
